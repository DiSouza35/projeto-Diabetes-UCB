...
#schedule_interval="*/3 * * * * "
dag = DAG('diabetes', description='Dados da diabetes',
          schedule_interval=None, start_date=datetime(2023,3,5),
          catchup=False, default_args=default_args, default_view='graph',
          doc_md="## Dag para registrar dados ")
...
send_email_alert = EmailOperator(
                                ...
                                html_content = '''<h3>Alerta de diabetes. </h3>
                                <p> Dag: diabetes </p>
                                ''',
...
send_email_normal = EmailOperator(
                                ...
                                html_content = '''<h3>diagnosticos normais. </h3>
                                <p> Dag: diabetes </p>
                                ''',
                                task_group = group_check_temp,
                                dag=dag)
...
def avalia_temp(**context):
    number = float( context['ti'].xcom_pull(task_ids='get_data', key="diagnosticos"))
    if number >= 24 :
